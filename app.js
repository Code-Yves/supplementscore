...
function _showSuppShareToast(msg){const t=document.getElementById('supp-share-toast');if(!t)return;t.innerHTML='<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>'+escHtml(String(msg||''));t.classList.add('show');clearTimeout(_showSuppShareToast._t);_showSuppShareToast._t=setTimeout(()=>{t.classList.remove('show');},1800);}
...
const POPULATIONS={
  ...
  men_50_64:{label:'Men (50–64)',supps:['Vitamin D3','Omega-3 (EPA/DHA)','Magnesium','Creatine monohydrate','Whey protein','CoQ10 (Ubiquinol)','Vitamin K2 (MK-7)','Zinc','Saw palmetto (Serenoa repens)','Curcumin (bioavailable form)','Boswellia serrata','Vitamin B12','Berberine','Fibre (general dietary)','Psyllium husk (Plantago ovata)','Probiotics','B-complex (balanced)','Multivitamins (healthy adults)','Astaxanthin','Collagen peptides']},
  women_50_64:{label:'Women (50–64)',supps:['Vitamin D3','Calcium','Magnesium','Omega-3 (EPA/DHA)','Vitamin K2 (MK-7)','Collagen peptides','Creatine monohydrate','Whey protein','CoQ10 (Ubiquinol)','Vitamin B12','Curcumin (bioavailable form)','Boswellia serrata','Iron','Black cohosh (Cimicifuga racemosa)','Soy isoflavones','Berberine','Probiotics','Fibre (general dietary)','B-complex (balanced)','Multivitamins (healthy adults)','Astaxanthin','Psyllium husk (Plantago ovata)']},
...
const POP_META={
  ...
  men_50_64:{lead:'Midlife maintenance and prevention',desc:'Supports muscle preservation, cardiometabolic and prostate health, joints, and bone as men move through their 50s and early 60s.',icon:'<circle cx="12" cy="8" r="4"/><path d="M4 22v-1a8 8 0 0116 0v1"/>'},
  women_50_64:{lead:'Midlife, bone, and heart health',desc:'Supports bone density, cardiometabolic health, muscle maintenance, and the menopausal transition for women in their 50s and early 60s.',icon:'<circle cx="12" cy="8" r="4"/><path d="M4 22v-1a8 8 0 0116 0v1"/><path d="M7 4c0-2 2-3 5-3s5 1 5 3"/>'},
...
/* Round-12: condition/symptom filters were listing every keyword match (140+ for
   Cardiovascular, 100+ for Memory/Blood sugar). Cap to the top, well-evidenced
   supplements (composite score ≥ 65) so both the dropdown counts and the results
   show the strongest options, not the long tail. Floor inlined in _sxCount + _sxPick. */
const _sxCount=(v)=>{const ks=(typeof SX_KEYWORDS!=='undefined'&&SX_KEYWORDS[v])||[];if(!ks.length)return 0;return S.filter(s=>{if(s.t==='t4')return false;if(typeof calcScore==='function'&&calcScore(s)<65)return false;const hay=((s.tag||'')+' '+(s.desc||'')).toLowerCase();return ks.some(k=>hay.indexOf(k)!==-1);}).length;};
...
function _sxPick(v){
  ...
  _ddLabel('cat-filter','Goal');_ddLabel('pop-filter','Age & Sex');_ddLabel('art-filter','Research');
  document.querySelectorAll('.sfbtn').forEach(function(b){b.className='sfbtn';});
  af='sx';
  var items=S.filter(function(s){
    if(s.t==='t4')return false;
    if(typeof calcScore==='function'&&calcScore(s)<65)return false; /* Round-12: top-scoring only (≥65) — keep counts (_sxCount) and results in sync */
...
function _tierPick(v){_ddLabel('tier-filter',v==='unproven'?'Unproven':(TIERS.find(x=>x.id===v)||{}).badge||'Tier');document.querySelectorAll('.sfbtn').forEach(b=>b.className='sfbtn');_ddLabel('cat-filter','Goal');_ddLabel('az-filter','A\u2013Z');_ddLabel('pop-filter','Age & Sex');_ddLabel('art-filter','Research');_ddActive('tier-filter');af=v;renderAll();if(_initialLoad){_initialLoad=false;return;}const content=document.getElementById('s-content');if(content){const stickyH=document.querySelector('.sticky-bar');const offset=stickyH?stickyH.offsetHeight+76:120;const top=content.getBoundingClientRect().top+window.pageYOffset-offset;window.scrollTo({top:top,behavior:'smooth'});}}
...
function _catPick(v){
  if(v==='__trending__'){
    ...
    _ddLabel('pop-filter','Age & Sex');_ddLabel('art-filter','Research');
