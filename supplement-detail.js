...
(function(){
  ...
  var sourcesHtml = '';
  if (artiles.length){
    /* Article URL resolution (2026-06-06) — link STRAIGHT to the article's
       full /a/ page when one exists. a/manifest.js (loaded by supplement.html,
       regenerated by scripts/gen_article_manifest.py) maps id → filename.

       The previous routing bounced through index.html?supplement=<slug>
       #article-<id>: the homepage loaded, supplement-modal.js auto-opened the
       supplement modal ("Loading…"), and ~120ms later app.js's hash handler
       hit the slimmed stub and redirected to the /a/ page anyway. The user
       saw a modal flash open and close between click and article — and on
       slow connections, a stuck-looking "Loading…" overlay.

       Return flow is native now: opening the supplement modal pushState'd
       ?supplement=<slug>, so the browser Back button from the /a/ page lands
       on that URL and the supplement modal auto-reopens.

       target="_top" breaks out of the supplement-modal iframe so the article
       loads in the parent window; standalone supplement.html behaves the same.

       Page-less articles (a handful of homepage-only entries with no /a/
       file, hence no manifest entry) keep the legacy roundtrip URL — for
       them index.html genuinely renders the art-modal on top of the
       reopened supplement modal. */
    sourcesHtml = '<div class="det-sources" id="det-sources"><div class="det-sources-h">Sources &amp; further reading</div><div class="det-sources-grid">'
      + artiles.map(function(a){
          ...
          var file = null;
          try {
            var man = window.__ART_MANIFEST || {};
            file = man[String(a.id)] || man[a.id] || null;
          } catch(_){}
          var artUrl = file
            ? '/a/' + file
            : 'index.html?supplement=' + encodeURIComponent(slug)
              + '#article-' + encodeURIComponent(a.id);
          return '<a href="'+escAttr(artUrl)+'" target="_top">'
  ...
  /* Back link removed 2026-06-06 — it duplicated the close-X (.pg-close-fab),
     which already history.back()s. The floating X (top-right) is now the single
     close/return control; on direct entry with no history it falls back to the
     homepage. (The "not found" error path below keeps its own search link.) */
  root.innerHTML =
